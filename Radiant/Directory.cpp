...
namespace Radiant
{
  ...
  bool Directory::applyFilters(const struct dirent * dent) {
    ...
    if(dent->d_type == DT_DIR && !(m_filterFlags & Dirs)) ok = false;
    if(dent->d_type == DT_REG && !(m_filterFlags & Files)) ok = false;
    if( (name == DOT || name == DOTDOT) && (m_filterFlags & NoDotAndDotDot)) 
        ok = false;
